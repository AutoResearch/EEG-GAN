...
def load_and_corr(filenames, electrode, antisaccade):
    ...
    return {'GAN': corr_gan_diff, 'VAE': corr_vae_diff}
...
def main():
    ...
    correlation_dataframe = correlation_dataframe.round(2)
    correlation_dataframe.reset_index(inplace=True)
    correlation_dataframe.loc[correlation_dataframe['index']=='blank', 'index'] = ''
    correlation_dataframe.rename(columns={'index':'Dataset'}, inplace=True)
    ...
    correlation_dataframe.to_latex('evaluation/grand_averaged_correlations.tex', index=False, caption=caption, label='tab-0')
