...
class Trainer:
            ...
            for i in range(0, len(sessions), self.batch_size):
                ...
                if int(i / batch_size) % self.critic_iterations == 0:
                    ...
                    if isinstance(self.discriminator, models.TtsDiscriminator):
                        fake_labels = gen_labels.view(-1, 1, 1, 1).repeat(1, 1, 1, gen_imgs.shape[3])
                        fake_data = torch.cat((gen_imgs, fake_labels), dim=1)
                        validity = self.discriminator(fake_data)
                    else:
                        validity = self.discriminator(gen_imgs, gen_labels)
                    g_loss = self.generator_loss(validity)
                    g_loss.backward()
                ...
                if isinstance(self.generator, models.TtsGenerator):
                    ...
                    real_labels = data_labels.view(-1, 1, 1, 1).repeat(1, 1, 1, gen_imgs.shape[3])
                ...
                gp = self.calculate_gradient_penalty(data, gen_imgs, data_labels, gen_labels)
                d_loss = self.critic_loss(validity_real, validity_fake, gp.data)
                d_loss.backward()
                ...
                print(
                    ...
                    % (epoch, self.epochs,
                       int(i / self.batch_size), int(len(sessions) / self.batch_size),
                       d_loss.item(), g_loss.item())
                )

                # Save a checkpoint of the trained GAN and the generated samples every sample interval
                batches_done = epoch * int(len(sessions) / self.batch_size) + int(i / self.batch_size)
                if batches_done % self.sample_interval == 0:
                    ...
                    checkpoint = {
                        ...
                        'optimizer_generator': self.generator_optimizer.state_dict(),
                        'optimizer_discriminator': self.discriminator_optimizer.state_dict(),
                        'generated_samples': gen_samples
                    }
                    torch.save(checkpoint, r'trained_models\checkpoint.pth')
    ...
    @staticmethod
    def critic_loss(real, fake, gradient_penalty):
        """Critic loss function for Wasserstein GAN with GP."""
        return fake.mean() - real.mean() + gradient_penalty
        # return torch.mean(fake) - torch.mean(real)

    @staticmethod
    def generator_loss(fake):
        """Generator loss function for Wasserstein GAN."""
        return -torch.mean(fake)

    def calculate_gradient_penalty(self, real_images, fake_images, real_labels, fake_labels):
        ...
        if isinstance(self.discriminator, models.TtsDiscriminator):
            real_labels = real_labels.view(-1, 1, 1, 1).repeat(1, 1, 1, real_images.shape[3])
            fake_labels = fake_labels.view(-1, 1, 1, 1).repeat(1, 1, 1, real_images.shape[3])
            interpolated_labels = (eta * real_labels + ((1 - eta) * fake_labels))
        else:
            interpolated_labels = (eta * real_labels + ((1 - eta) * fake_labels))\
                .unsqueeze(-1).repeat(1, interpolated.shape[1], 1)

        interpolated = torch.concat((interpolated, interpolated_labels), dim=1)
        ...
        gradients = autograd.grad(outputs=prob_interpolated, inputs=interpolated,
                                  grad_outputs=torch.ones(prob_interpolated.size(), device=self.device, requires_grad=False),
