...
def main(multiprocessing, features, validationOrTest, dataSampleSizes, syntheticDataOptions, classifiers, num_series):
...
def run_classification(q, multiprocessing, validationOrTest, features, electrode_number, classifier, addSyntheticData, dataSampleSize, series, run, y_test, x_test):
    ...
    #Load empirical data
    tempFilename = f'data/ERPCORE/N400/Training Datasets/erpcore_N400_SS{dataSampleSize}_Run0{run}.csv'
...
if __name__ == '__main__':
    ...
    for i, current_classifiers in enumerate([mp_classifiers, nmp_classifiers]):
        if current_classifiers: 
            ...
            main(multiprocessing, features, validationOrTest, dataSampleSizes, syntheticDataOptions, current_classifiers, num_series)
