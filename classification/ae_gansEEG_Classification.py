...
syntheticDataOptions = [0, 2] #The code will iterate through this list. 0 = empirical classifications, 1 = augmented classifications, 2 = autoencoder
classifiers = ['SVM', 'LR', 'NN'] #The code will iterate through this list
remove_trials_above = 5
###############################################
## SETUP                                     ##
###############################################

#Base save file names
augFilename = 'Classification Results/augmentedPredictions_XX_SynP050.csv'
aeFilename = 'Classification Results/autoencoderPredictions_XX.csv'
empFilename = 'Classification Results/empiricalPredictions_XX.csv'
...
def encodeData(ae_filename, data_filename, remove_trials_above = None):

    #Load data
    dataloader = Dataloader(data_filename, col_label='Condition', channel_label='', remove_trials_above=remove_trials_above)
    eeg = dataloader.get_data(shuffle=False)#[:,1:]

    #Load autoencoder
    device = torch.device('cpu')
    ae_dict = torch.load(ae_filename, map_location=device)
...
for classifier in classifiers: #Iterate through classifiers (neural network, support vector machine, logistic regression)
    ...
    for addSyntheticData in syntheticDataOptions: #Iterate through analyses (empirical, augmented)
        ...
        if addSyntheticData == 2:
            ...
            autoencoder = False #Whether to implement autoencoder classification
        ...
        for dataSampleSize in dataSampleSizes: #Iterate through sample sizes   
            for run in range(5): #Conduct analyses 5 times per sample size
                ...
                if remove_trials_above is not None:
                    EEGDataTest = EEGDataTest[EEGDataTest[:,2]<=remove_trials_above,:]
                ...
                if autoencoder:
                    if validationOrTest == 'validation':
                        ...
                        EEGDataTest = encodeData(f'../trained_ae/ae_ddp_1000ep_SS{dataSampleSize}_Run0{run}_enc25.pt', test_fn, remove_trials_above)
                ...
                if remove_trials_above is not None:
                    EEGData = EEGData[EEGData[:,2]<=remove_trials_above,:]
                
                if addSyntheticData == 2:
                    aeData = encodeData(f'../trained_ae/ae_ddp_1000ep_SS{dataSampleSize}_Run0{run}_enc25.pt', tempFilename, remove_trials_above)
