            ...
            if np.random.rand() < .5: #Only half are transformed
                for e in range(x_.shape[-1]):
                    ...
                    if addSyntheticData == 'gaus':
                        ...
                        mask_len_samples = int((np.random.uniform()*125)/6.67)+10 # 10-25% of the data
                        start_location = np.random.choice(range(0,125-mask_len_samples))
...
if __name__ == '__main__':
    ...
    classifiers = ['SVM'] #The code will iterate through this list #NOTE NN AND LR USE THEIR OWN MULTIPROCESSING AND SLOWS THINGS WHEN RUN WITH MP, SO SHOULD BE RUN ONLY ALONE OR TOGETHER
