...
class Trainer:
    ...
    def training(self, dataset):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, dataset.shape[0], self.batch_size):
                ...
                d_loss, g_loss, c_loss, gen_imgs = self.batch_train(data, data_labels, train_generator)
            ...
            self.print_log(epoch + 1, d_loss, g_loss, c_loss)
    ...
    def batch_train(self, data, data_labels, train_generator):
        ...
        # ---------------------
        #  Train Classifier on Generator Data
        # ---------------------

        self.classifier_optimizer.zero_grad()

        labels = validity_fake.view(-1, data_labels.shape[1], 1, 1).repeat(1, 1, 1, self.sequence_length).to(self.device)

        classifier_data = torch.cat((fake_data, labels), dim=1).to(self.device)
        # validity_classifier = self.classifier(classifier_data)
        # x = torch.empty_like(gen_imgs)
        x = torch.empty_like(classifier_data)
        print(x)
        output = self.classifier(x)
        print(output)
        # loss = self.loss_fn(output.view(output.shape[0], output.shape[-1]), torch.empty_like(labels))
        c_loss = self.loss.classifier(output)
        print(c_loss)
        # c_loss = loss
        # self.classifier_optimizer.zero_grad()
        c_loss.backward()
        ...
        # ---------------------
        #  Train Generator on Classifier Loss
        # ---------------------

        # Must check that this works as intended! It probably doesn't
        # self.generator_optimizer.zero_grad()
        #
        # # alpha = abs(float(d_loss.item())) / abs(float(g_loss.item()))  # This balances classifier training with overall GAN training
        # alpha = 0.1
        # gc_loss = c_loss * alpha
        # gc_loss.backward()
        # self.generator_optimizer.step()
        #
        # g_loss = self.loss.classification(output)
        # g_loss.backward()
        # self.generator_optimizer.step()
        #
        # g_loss += gc_loss

        return d_loss.item(), g_loss, c_loss.item(), gen_samples
    ...
    def load_checkpoint(self, path_checkpoint):
        if os.path.isfile(path_checkpoint):
            ...
            print(f"Device {self.device}:{self.rank}: Using pretrained GAN Classifier.")
        else:
    ...
    def manage_checkpoints(self, path_checkpoint: str, checkpoint_files: list, generator=None, discriminator=None):
        ...
        self.save_checkpoint(path_checkpoint=os.path.join(path_checkpoint, 'checkpoint.pt'), generator=generator,
    ...
    def print_log(self, current_epoch, d_loss, g_loss, c_loss):
        print(
            "[Epoch %d/%d] [D loss: %f] [G loss: %f] [C loss: %f]"
            % (current_epoch, self.epochs,
               d_loss, g_loss, c_loss)
        )

    def set_optimizer_state(self, optimizer, g_or_d_or_c='G'):
        if g_or_d_or_c == 'G':
            ...
            self.classifier_optimizer.load_state_dict(optimizer)
            print('Discriminator optimizer state loaded successfully.')
        else:
            raise ValueError('G_or_D must be either "G" (Generator) or "D" (Discriminator) or "C" (Classifier)')
