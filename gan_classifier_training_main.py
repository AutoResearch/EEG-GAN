...
from helpers.trainer_gc import Trainer
from helpers.get_master import find_free_port
from nn_architecture.models import TtsDiscriminator, TtsGenerator, TtsGeneratorFiltered, TtsClassifier
...
if __name__ == '__main__':
    ...
    # sys.argv = ["path_dataset=data/ganAverageERP_multiCond_small.csv", "n_epochs=2", "patch_size=20",
    # "conditions=Condition_1,Condition_2,Condition_3", "sample_interval=1"]
    # sys.argv = ["path_dataset=data/ganTrialElectrodeERP_mini.csv", "n_epochs=20", "patch_size=20", "batch_size=7"]
    sys.argv = ["path_dataset=data/gansEEGValidationData.csv", "n_epochs=20", "patch_size=20", "batch_size=2"]
    ...
    classifier = TtsDiscriminator(seq_length=opt['sequence_length'], patch_size=opt['patch_size'], in_channels=3)
    # classifier = TtsClassifier(seq_length=opt['sequence_length'],
    #                            patch_size=opt['patch_size'],
    #                            n_classes=int(opt['n_conditions']),
    #                            softmax=True).to(device)
    print("Generator, discriminator, and classifier initialized.")
    ...
    if train_gan:
        ...
        if ddp:
            ...
            trainer = Trainer(generator, discriminator, classifier, opt)
